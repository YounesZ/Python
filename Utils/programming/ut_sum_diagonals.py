...
def main(matrix):
    ...
    startI  =   np.concatenate( (range(shp[0]-1,-1,-1), range(shp[0],np.prod(shp)-1,shp[1])), axis=1 )
    nInd    =   np.concatenate( (range(shp[0]), range(shp[0]-2,-1,-1)), axis=1 )
    lElem   =   np.multiply(range(shp[0]), shp[0]+1)
    for ii  in range(nDiag):
        # list all indices
        allI    =   startI[ii] + lElem[:nInd[ii]+1]
        # Convert to subscripts
        r,c     =   ut_ind2sub.main(shp, allI)
        sumD[0,ii] = np.sum( [matrix[r[xxx]][c[xxx]] for xxx in range(len(r))] )
