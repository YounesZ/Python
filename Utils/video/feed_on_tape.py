...
def read_feeddata(datapath, season, gcode):
    # Read data
    rawDT   =   pickle.load( open(path.join(datapath, 'GAME_data.p'), 'rb') )['GAME_data']
    procDT  =   pickle.load( open(path.join(datapath, 'RL_teaching_data.p'), 'rb') )
    nAct    =   procDT['nActions']
    procDT  =   procDT['RL_data']
...
# Prepare video
w,h,r,d,nf  =   read_gametape(videopath)
